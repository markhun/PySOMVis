...
class MinimumSpanningTree(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = MinimumSpanningTreeController(
            self._calculate, name="Minimum Spanning Tree"
        )
    ...
    def _activate_controllers(
        ...
        reference = pn.pane.Str(
            '<ul><li>"Visualising Clusters in Self-Organising Maps with Minimum SpanningTrees." <b>Mayer, Rudolf, and Andreas Rauber.</b> International Conference on Artificial Neural Networks. Springer, Berlin, Heidelberg, 2010.</li></ul>'
        )
        self._old_coolormap = self._main._maincontrol.colormap
        self._main._maincontrol.colormap = "RdGy"
        self._main._display(plot=[])
        self._main._controls.append(
            pn.Column(self._controls, self._main._point_segment_options, reference)
        )
    ...
    def _calculate(self, connection_type=0, weighted_lines=False):
        ...
        if connection_type == 1:
            self._diagonal_edges(G)
        if connection_type == 2:
            self._direct_edges(G)
        if connection_type == 3:
            self._input_data_edges(G)
        ...
        for n1, n2, w in MST:
            paths.append(
                list(
                    self._main._convert_to_xy(neuron=n1)
                    + self._main._convert_to_xy(neuron=n2)
                )
                + [w["weight"]]
            )
    ...
    def _input_data_edges(self, G):
        for v1 in range(0, len(self._main._idata)):
            for v2 in range(v1 + 1, len(self._main._idata)):
                ...
                n1 = np.argmin(
                    np.linalg.norm(self._main._weights - self._main._idata[v1], axis=1)
                )
                n2 = np.argmin(
                    np.linalg.norm(self._main._weights - self._main._idata[v2], axis=1)
                )
                if n1 != n2:
    ...
    def _diagonal_edges(self, G):
        for y in range(0, self._main._m):
            for x in range(0, self._main._n):
                n = self._idx(y, x)
                if n < (
                    self._main._m * self._main._n - 1
                ):  # last neuron has all connections
                    if y == self._main._m - 1 and x < self._main._n - 1:
    ...
    def _add_edge(self, G, n1, n2):
        is_activated_unit = lambda v1, v2: (v1 in self._activated_unites) and (
            v2 in self._activated_unites
        )  # if consider only activated unites
        distance = lambda v1, v2: np.linalg.norm(
            self._main._weights[v1] - self._main._weights[v2]
        )  # calculate weight between two neurons

...
class ClusterConnection(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = ClusterConnectionController(
            self._calculate, name="Cluster connection"
        )
        self._alpha = []
        self._old_coolormap = ""
    ...
    def _activate_controllers(
        ...
        reference = pn.pane.Str(
            '<ul><li>"Alternative ways for cluster visualization in self-organizing maps." <b>Merkl, Dieter, and Andreas Rauber.</b> Proc Workshop on Self-Organizing Maps (WSOM97), Espoo, Finland</li></ul>'
        )
    ...
    def _calculate(self, paths, t1, t2, t3):
        ...
        if np.any(mask):
            new_alpha.iloc[mask[0], 4] = 0.1

        mask = np.where(self._alpha > t1) and np.where(self._alpha < t2)
        if np.any(mask):
            new_alpha.iloc[mask[0], 4] = 0.45

        mask = self._alpha < t1
        if np.any(mask):
            new_alpha.iloc[mask, 4] = 1
    ...
    def _get_paths(
        ...
        self._alpha = np.interp(
            self._alpha, (min(self._alpha), max(self._alpha)), (0, 2)
        )
        self._calculate(
            pd.DataFrame(paths, columns=["x0", "x1", "y0", "y1", "alpha"]),
            self._controls.t1,
            self._controls.t2,
            self._controls.t3,
        )

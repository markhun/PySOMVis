...
class Clustering(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = ClusteringController(
            self._calculate, name="Clustering visualization"
        )
    ...
    def _activate_controllers(
        ...
        reference = pn.pane.Str(
            '<ul><li><b>D. Merkl and A. Rauber.</b> "Alternative ways for cluster visualization in self-organizing maps." In Proc. of the Workshop on Self-Organizing Maps (WSOM97), Helsinki, Finland, 1997.</li></ul>'
        )
    ...
    def _AgglomerativeClustering(
        self,
    ):
        agglo = AgglomerativeClustering(
            n_clusters=self._controls.clusters, linkage=self._controls.linkage_type
        )

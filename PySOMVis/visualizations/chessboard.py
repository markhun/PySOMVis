...
class Chessboard(VisualizationInterface):
    ...
    def _calculate(
        ...
        bmu = np.apply_along_axis(
            lambda x: np.argmin(
                np.linalg.norm(
                    self._main._weights - x.reshape((1, self._main._dim)), axis=1
                )
            ),
            1,
            self._main._idata,
        ...
        for position, v in enumerate(mapped_data):
            if sum(v) > 0:
                ...
                if x == 0:
                    dummy.append([-1, y])
                if x == self._main._n - 1:
                    dummy.append([self._main._n, y])
                if y == 0:
                    dummy.append([x, -1])
                if y == self._main._m - 1:
                    dummy.append([x, self._main._m])
                if x == 0 and y == 0:
                    dummy.append([-1, -1])
                if x == self._main._n - 1 and y == self._main._m - 1:
                    dummy.append([self._main._n, self._main._m])
                if x == self._main._n - 1 and y == 0:
                    dummy.append([self._main._n, -1])
                if x == 0 and y == self._main._m - 1:
                    dummy.append([-1, self._main._m])
        ...
        for position in range(len(dummy), len(vpoints)):
            ...
            perc = np.round(
                colors / np.linalg.norm(colors, 1.0), 3
            ...
            colors_ind = colors_ind[
                colors_ind != dominant_color
            ]  # we don't need dominant color any more
            if len(colors_ind) > 0:
                ...
                if self._controls.chessboard:
                    ...
                    chunks = np.split(
                        pixels, (len(pixels) * perc[:-1].cumsum()).astype(int)
                    )  # split pixels accordingly percentages
                    for c, pixels in enumerate(chunks):
                        plot[pixels[:, 0], pixels[:, 1]] = c
                else:
                    # find neighbours of current polygon
                    neighbours = vdiagram.ridge_points[
                        (vdiagram.ridge_points == position).any(axis=1)
                        & (vdiagram.ridge_points >= len(dummy)).all(axis=1)
                    ]
                    neighbours = neighbours[
                        neighbours != position
                    ].flatten()  # remove the current region form neighbours

                    # get colors of all neighbors
                    neighbours_colors = np.insert(
                        mapped_data[neighbours - len(dummy)],
                        mapped_data.shape[1],
                        neighbours,
                        axis=1,
                    )
                    ...
                    for c in colors_ind:  # go over all colors of polygon
                        pixels = pixels[
                            plot[pixels[:, 0], pixels[:, 1]] == dominant_color
                        ]  # we can change only pixels colored in dominant color
                        point1, point2 = None, None

                        nc = neighbours_colors[
                            neighbours_colors[:, c] > 0
                        ]  # check how many neighbours have the same colour
                        if len(nc) == 0:
                            point1, point2 = (
                                centroid,
                                None,
                            )  # color is isolated, no neighbours with the same color
                        elif (
                            len(nc) == 1
                        ):  # there is only one neighbour with the same color
                            neighbour = get_vertices(nc[:, -1][0])
                            adjacent_border = get_coordinates(
                                np.intersect1d(polygon_cur, neighbour)
                            )
                            ...
                            if nc[nc > 0].size - 1 == 1:
                                point1, point2 = (
                                    adjacent_border[1],
                                    adjacent_border[0],
                                )  # if this color is dominant color for neighbor - consider adjacent border
                            else:
                                point1, point2 = centroid, adjacent_border.mean(
                                    0
                                )  # else consider line segment from center to border

                        elif (
                            len(nc) > 1
                        ):  # if we have a set of neighbours with the same color
                            dominant = []
                            not_dominant = []
                            for n in nc:  # divide set on dominant and not
                                if n[n > 0].size - 1 == 1:
                                    ...
                                    not_dominant.append(n[-1])

                            if (
                                len(dominant) > 0
                            ):  # consider dominant color of neighbors - first
                                intersection = self._check_polygon_intersection(
                                    dominant, get_vertices
                                )
                                if (
                                    intersection is not None
                                ):  # check iintersection of neighbors with the same color
                                    point1, point2 = (
                                        get_coordinates(intersection[0]),
                                        None,
                                    )
                                else:
                                    neighbour = get_vertices(
                                        dominant[0]
                                    )  # no intersection - take first neighbour
                                    adjacent_border = get_coordinates(
                                        np.intersect1d(polygon_cur, neighbour)
                                    )
                                    point1, point2 = (
                                ...
                                intersection = self._check_polygon_intersection(
                                    not_dominant, get_vertices
                                )
                                if intersection is not None:
                                    point1 = get_coordinates(
                                        np.intersect1d(intersection[1], polygon_cur)
                                    ).mean(0)
                                    point2 = get_coordinates(
                                        np.intersect1d(intersection[2], polygon_cur)
                                    ).mean(0)
                                else:
                                    neighbour1 = get_vertices(not_dominant[0])
                                    neighbour2 = get_vertices(not_dominant[-1])
                                    point1 = get_coordinates(
                                        np.intersect1d(neighbour1, polygon_cur)
                                    ).mean(0)
                                    point2 = get_coordinates(
                                        np.intersect1d(neighbour2, polygon_cur)
                                    ).mean(0)

                        sorted_pixels = self._dist_point_to_segment(
                            pixels, point1, point2
                        )

                        N = round(
                            total_pixels * perc[c]
                        )  # get number of pixels accordying to the percentge
                        plot[
                            sorted_pixels[0:N, 0], sorted_pixels[0:N, 1]
                        ] = c  # cahnge pixel color
        ...
        if self._controls.voronoi_lines:
            for v in vdiagram.ridge_vertices:
                if -1 not in v:
                    p1 = list(
                        self._main._convert_to_xy(point2D=vdiagram.vertices[v[0]])
                    )
                    p2 = list(
                        self._main._convert_to_xy(point2D=vdiagram.vertices[v[1]])
                    )
    ...
    def _dist_point_to_segment(self, pixels, s0, s1=None):
        ...
        if s1 is None:  # distance between points and point
            ...
            for p in pixels:
                ...
                pb = s0 + b * v
                if c1 <= 0:
                    distances.append(dist(p, s0))
                elif c2 <= c1:
                    distances.append(dist(p, s1))
                else:
                    distances.append(dist(p, pb))
    ...
    def _check_polygon_intersection(self, polygons, get_vertices):
        ...
        for i in range(len(polygons)):
            for j in range(i + 1, len(polygons)):
                ...
                if shared_vertex.size == 1:
                    return (shared_vertex, poly1, poly2)
        return None

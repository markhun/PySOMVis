            ...
            for idx, name in enumerate(self._main._classes_names):
                ColorColumn.append(
                    pn.widgets.ColorPicker(
                        name=name, value=_COLOURS_93[idx], disabled=True
                    )
                )
    ...
    def _get_pie_chart(self, x, y, piechart, max_size):
        ...
        if wedge_n > 0:
            ...
            if len(wedge) >= 3:
                color = _COLOURS_93[
                    np.where(wedge == True)
                ]  # np.array(Category20[len(wedge)])[wedge]
            else:
                color = _COLOURS_93[
                    np.where(wedge == True)
                ]  # np.array(Category20[3][:len(wedge)])[wedge]
            return (
                np.array([x] * wedge_n),
                np.array([y] * wedge_n),
                color,
                np.array([total / max_size * PIE_CHART] * wedge_n),
                angle_start,
                angle_end,
            )
        return (

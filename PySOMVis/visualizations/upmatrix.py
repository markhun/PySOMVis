...
from . import umatrix as UMatrix
...
class UStar_PMatrix(VisualizationInterface):
    def __init__(self, main):
        ...
        self._controls = UStar_PMatrixController(
            percentile,
            radius,
            self._calculate,
            self._calculate_optimal,
            self._percentiles,
            name="P-Matrix & U*-Matrix",
        )
    ...
    def _activate_controllers(
        ...
        reference = pn.pane.Str(
            '<ul><li><b>P-Matrix:</b> Ultsch, A. "Maps for the Visualization of high-dimensional Data Spaces." In Proceedings Workshop on Self-Organizing Maps (WSOM 2003), Kyushu, Japan</li> <li><b>U*-Matrix:</b> Ultsch, A.  "A Tool to visualize Clusters in high dimensional Data." Technical Report No. 36, Dept. of Mathematics and Computer Science, University of Marburg, Germany, 2003</li></ul>'
        )
        self._main._controls.append(
            pn.Column(self._controls, reference, name="")
    ...
    def _calculate(
        ...
        distance = lambda v: np.sum(
            np.linalg.norm(self._main._idata[:, None, :] - v, axis=-1)
            < self._controls.radius
        )
        plot = np.array([distance(v) for v in self._main._weights])
        if self._controls.ustar_matrix:
            plot = self._calculate_ustar_matrix(plot)
        self._main._display(plot=plot.reshape(self._main._m, self._main._n))

    def _calculate_ustar_matrix(self, matrix):
        um = UMatrix.calculate_UMatrix(
            self._main._weights, self._main._m, self._main._n, self._main._dim
        )
    ...
    def _calculatePercentiles(
        ...
        return np.quantile(
            self._main._distance[np.triu_indices(self._main._distance.shape[0], k=1)],
    ...
    def calculateParetoRadiusPercentile(
        self,
    ):
        PARETO_SIZE = 0.2013
        ...
        while not stop:
            ...
            if len(densities) > 0:
                median_size = max(np.median(densities), np.mean(densities)) / (
                    self._main._n * self._main._m
                )
            else:
                median_size = 0
            ...
            stop = (
                abs(percentile - last_percentile) == 1

...
class MetroMap(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = MetroMapController(
            self._calculate,
            self._main._dim,
            self._main._component_names,
    ...
    def _deactivate_controllers(
        ...
        self._main._pdmap[0] = (
            self._main._Image.apply.opts(
                cmap=self._main._maincontrol.param.colormap, color_levels=None
            )
            * self._main._Paths
        )
    ...
    def _calculate(self, calculating):
        ...
        if len(self._controls.components_int) != 1:
            data = hv.Image(self._main._pipe.data).apply.opts(
                cmap="Blues",
                color_levels=self._controls.param.water_level,
            ...
            data = hv.Image(
                self._digitize(self._controls.components_int[0])
            ).apply.opts(
                cmap="jet",
                width=self._main._width,
                height=self._main._height,
        ...
        for i, pts in enumerate(lines):
            ...
            overlay.append(
                hv.Points(np.array(pts)).opts(fill_color=c, color="black", size=6)
            )
    ...
    def _adjust_cordinates(self, lines):
        ...
        for feature in range(lines.shape[0]):
            for point in range(lines.shape[1]):
                lines[feature, point] = self._main._convert_to_xy(
                    point2D=lines[feature, point]
                )
        return lines
    ...
    def _find_snapped_line(self, line):
        ...
        for i, pt in enumerate(line):
            for point in self._get_neighbours(pt.tolist()):
                if i >= 1:
                    if i < length:
                        lineSegmentForward = self._doSnapping(point, i, 1, line)
                    if i > 0:
                        lineSegmentBackward = self._doSnapping(point, i, -1, line)
                    if i > 0 and i < length:
                        for j in range(length):
                            if lineSegmentForward[0][j] == -1:
                                lineSegmentForward[0][j] = lineSegmentBackward[0][j]
                            if lineSegmentBackward[0][j] == -1:
                                lineSegmentBackward[0][j] = lineSegmentForward[0][j]
                    if i == length - 1:
                        lineSegmentForward = lineSegmentBackward
    ...
    def _get_closest_points(self, pt1, pt2):
        ...
        for i in range(1, max(self._main._m, self._main._n)):
            ...
            if pt1[0] - i >= 0 and pt1[1] - i >= 0:
                points.append([pt1[0] - i, pt1[1] - i])

            if pt1[0] + i < self._main._m and pt1[1] - i >= 0:
                points.append([pt1[0] + i, pt1[1] - i])
            if pt1[0] - i >= 0 and pt1[1] + i < self._main._n:
                points.append([pt1[0] - i, pt1[1] + i])
    ...
    def _get_neighbours(self, pt):
        ...
        neighbours = [pt]
        if pt[1] + 1 < self._main._m:
            neighbours.append([pt[0], pt[1] + 1])
        if pt[0] + 1 < self._main._n:

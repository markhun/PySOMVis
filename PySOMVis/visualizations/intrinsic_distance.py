...
class IntrinsicDistance(VisualizationInterface):
    ...
    def _calculate(self):
        intrinsic_distance = np.zeros(self._main._m * self._main._n).reshape(
            self._main._m, self._main._n
        )
        quant_error = np.zeros(self._main._m * self._main._n).reshape(
            self._main._m, self._main._n
        )
        result_error_map = np.zeros(self._main._m * self._main._n).reshape(
            self._main._m, self._main._n
        )
        n_best_matching = np.zeros(self._main._n * self._main._m).reshape(
            self._main._m, self._main._n
        )

        for vector in self._main._idata:
            vec_to_weights_dis: np.ndarray = np.sqrt(
                np.sum(np.power(self._main._weights - vector, 2), axis=1)
            ).reshape(self._main._m, self._main._n)
            idx_best: Tuple[int, int] = np.unravel_index(
                np.argmin(vec_to_weights_dis, axis=None), vec_to_weights_dis.shape
            )
            ...
            idx_2nd_best: Tuple[int, int] = np.unravel_index(
                np.argmin(vec_to_weights_dis_wihtout_best, axis=None),
                vec_to_weights_dis_wihtout_best.shape,
            )

            intrinsic_distance[idx_best] += self._calculate_distance(
                distances=vec_to_weights_dis,
                idx_best=idx_best,

...
class TopographicError(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = TopographicErrorController(
            self._calculate, name="Topograhpic Error"
        )
    ...
    def _calculate(self, neighborhood_type: int):
        ...
        *neighborhood_type* defines if the neighborhood is a 4-unit neighbhorhood (von neumann neighborhood) or a 8-unit neighborhood (moore neighborhood)
        ...
        for vector in self._main._idata:
            ...
            vec_to_weights_dis = np.linalg.norm(
                self._main._weights - vector, axis=1
            )  # np.sqrt(np.sum(np.power(self._main._weights - vector, 2), axis=1))
        ...
        topoerror[units_with_error_mask] = n_2nd_best_not_neighbor[
            units_with_error_mask
        ]

        # display only mapped neurons
        topoerror = topoerror.reshape(self._main._m, self._main._n) * hist.reshape(
            self._main._m, self._main._n
        )  # np.rot90(hist.reshape(self._main._m, self._main._n))
    ...
    def _is_in_neighborhood(
        self,
        pos_unit: Tuple[int, int],
        pos_other_unit: Tuple[int, int],

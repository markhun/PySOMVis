...
class ComponentPlane(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = ComPlaneController(
            self._calculate, (0, self._main._dim - 1), name="Component Planes"
        )
    ...
    def _calculate(self, component):
        if self._main._component_names is not None:
            self._main._controls[0][0][1].name = self._main._component_names[
                self._controls.component
            ]

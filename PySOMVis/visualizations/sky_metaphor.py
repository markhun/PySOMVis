...
class SkyMetaphor(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = SkyMetaphorController(
            self._calculate, name="SkyMetaphor visualization"
        )
    ...
    def _activate_controllers(
        ...
        reference = pn.pane.Str(
            '<ul><li>Sky-metaphor visualisation for self-organising maps.</b> Latif, K., Mayer, R. "I-KNOW \'07: 7th international conference on knowledge management." In Proc. of I-KNOW07, 2007, pp. 400-407, Graz, Austria</li></ul>'
        )
    ...
    def _calculate(self, smooth_factor=None, lam=None):
        ...
        if lam is not None:
            ...
            for vector in self._main._idata:
                ...
                xy_points = np.array(
                    [
                        [
                            int(i % self._main._n),
                            -1 * int(i / self._main._n) + self._main._m,
                        ]
                        for i in best_dists
                    ]
                )
            ...
            points[:, 0] = self.normalize(
                points[:, 0], [0, self._main._n], [-0.5, 0.5]
            ) + 1 / (self._main._n * 2)
            points[:, 1] = self.normalize(
                points[:, 1], [0, self._main._m], [-0.5, 0.5]
            ) - 1 / (self._main._m * 2)

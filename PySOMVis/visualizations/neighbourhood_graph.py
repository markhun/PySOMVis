...
class NeighbourhoodGraph(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = NeighbourhoodGraphController(
            self._calculate, name="Neighbourhood Graph"
        )
    ...
    def _calculate(self, method):
        if self._controls.approach == 0:
            self._neighbourhood_knn()
        else:
            self._neighbourhood_radius()

    def _neighbourhood_knn(
        self,
    ):
        bmu_array = np.apply_along_axis(
            lambda x: np.argmin(
                np.linalg.norm(
        ...
        neighbours = np.argpartition(self._main._distance, self._controls.knn)[
            :, : self._controls.knn + 1
        ]

        points = set()
        for index, neighbour_idxs in enumerate(neighbours):
            for neighbour_idx in neighbour_idxs:
                if index > neighbour_idx:
                    points.add((bmu_array[neighbour_idx], bmu_array[index]))
                if index < neighbour_idx:
                    points.add((bmu_array[index], bmu_array[neighbour_idx]))

        paths = []
        for p in points:
            paths.append(
                self._main._convert_to_xy(neuron=p[0])
                + self._main._convert_to_xy(neuron=p[1])
            )

        self._main._display(paths=paths)

    def _neighbourhood_radius(
        self,
    ):
        num_nodes = self._main._idata.shape[0]

        # for each input vector, get the assigned output unit (index), based on euclidean distance
        input_assigned_units = np.apply_along_axis(
            lambda x: np.argmin(
                np.linalg.norm(
                    self._main._weights - x.reshape((1, self._main._dim)), axis=1
                )
        ...
        filtered_by_radius_without_upper_triangle = np.tril(filtered_by_radius).astype(
            np.int
        )
        ...
        index_matrix = np.array(
            [
                list(range(0, num_nodes)),
            ]
            * num_nodes
        ).transpose()
        filtered_unit_index_matrix = np.multiply(
            filtered_by_radius_without_upper_triangle, index_matrix
        )

        # set irrelevant columns to -1
        result_matrix = np.where(
            filtered_unit_index_matrix > 0, filtered_unit_index_matrix, -1
        )

...
class MnemonicSOM(VisualizationInterface):
    def __init__(self, main):
        self._main = main
        self._controls = MnemonicSOMController(
            self._calculate, name="Mnemonic SOM visualization"
        )
    ...
    def _calculate(
        ...
        active_unit_matrix, self.distance_matrix = input_utils.load_mnemonic_image(
            self._controls.siluette,
            som_width=self._controls.N,
            som_height=self._controls.M,
        )
        ...
        for x in range(len(self.units)):
            for y in range(len(self.units[0])):
                if self.units[x][y]:
                    ret.append(self.units[x][y].get_weights())
                else:
                    ret.append(np.full(self._main._dim, 0))

        self._weights = np.array(ret)

        self._main._display(
            UMatrix.calculate_UMatrix(
                self._weights, self._controls.M, self._controls.N, self._main._dim
            )
        )
...
class SOMUnit:
    def __init__(
        ...
        self.learning_rate_decay = (
            learning_rate_decay if learning_rate_decay != 0 else 1
        )
    ...
    def adjust_weights(self, input_vector, epoch, distance_matrix, best_unit):
        self.weights += (
            self._learning_rate(epoch)
            * self._topological_distance(epoch, distance_matrix, best_unit)
    ...
    def _topological_distance(self, epoch, distance_matrix, best_unit):
        return math.exp(
            -(
                distance_matrix.get_distance_by_coords(
                    self.x, self.y, best_unit.x, best_unit.y
                )
                ** 2
            )
            / (2 * self._neighborhood_size(epoch) ** 2)
        )
    ...
    def _learning_rate(self, epoch):
        return self.initial_learning_rate * math.exp(
            -(epoch) / (self.learning_rate_decay)
        )
